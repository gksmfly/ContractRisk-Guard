...
> **후속 측정 있음 (2026-08-16)** — [`measurement_findings_2026-08-16.md`](measurement_findings_2026-08-16.md)
> 참고. 이 문서의 결론을 갱신하는 항목:
> - **라우팅 모델**: gpt-4o-mini로 교체 시 20%로 EXAONE(33%)보다 낮음(p=0.0044) → EXAONE 유지.
>   단 EXAONE 3.5는 **NC 라이선스**(상업적 사용 금지)이므로 수익화 시 교체 필요
> - **33%의 병목은 라우팅이 아니라 조문 랭킹**: 라우팅이 맞은 건의 98.7%는 정답이 이미
>   후보 풀에 있고 top-5 노출에서 놓칠 뿐. 라우팅을 100%로 만들어도 천장은 40%
> - **종합 콤보(28%)의 실패 원인 규명**: 재구성+라우팅을 **한 호출**에 시킨 것이 문제였고,
>   **호출을 분리하면 33% → 51%**(p=0.00012). 이 문서의 "합치면 낮아진다"는 관찰은
>   "한 호출에 합치면 낮아진다"로 정정됨
> - **평가와 운영의 노출 개수 불일치**: 이 문서의 모든 수치는 top-5 기준인데 운영은
>   top-2(`_FINAL_K=2`)라 실사용 정확도는 18%
