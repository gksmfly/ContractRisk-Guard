...
⚠️ 이 절의 전제는 **틀렸었다.** "학생이 gold에서 상수를 유의하게 이긴다(+9.1%p)"로
적혀 있었는데, 그 +9.1%p는 축2가 0.6%로 붕괴해 **기각한 `ratio 0.25` 팔**의 값이다.
방어 가능한 `ratio None`은 교차적합 +2.3%p [-2.0,+6.6]로 **미판정**이고, 배포 임계값
에서는 상수를 넘는 구성이 없다.

그래서 물음은 "천장이 어디인가"만이 아니라 **"전이가 됐나"도 아직 열려 있다**:
