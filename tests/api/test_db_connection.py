# tests/api/test_db_connection.py
...
class TestGetConn:
    def test_returns_connection_and_puts_back_on_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        conn = _FakeConn()
        pool = _FakePool(conn)
    ...
    def test_rolls_back_and_puts_back_on_exception(self, monkeypatch: pytest.MonkeyPatch) -> None:
        conn = _FakeConn()
        pool = _FakePool(conn)
        monkeypatch.setattr(connection, "_pool", pool)
