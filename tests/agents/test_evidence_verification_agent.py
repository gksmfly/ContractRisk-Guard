# tests/agents/test_evidence_verification_agent.py
"""
...
class TestEvidenceVerificationNode:
    def test_sufficient_evidence_stops_without_retry(self) -> None:
    ...
    def test_empty_legal_basis_triggers_retry(self) -> None:
        state = {"legal_basis": [], "evidence_agreement": False, "retry_count": 0}
        result = evidence_verification_node(state)
        assert result["should_retry"] is True
        assert result["retry_count"] == 1

    def test_no_agreement_triggers_retry(self) -> None:
        state = {"legal_basis": [object()], "evidence_agreement": False, "retry_count": 0}
        result = evidence_verification_node(state)
        assert result["should_retry"] is True
        assert result["retry_count"] == 1

    def test_retry_count_increments_each_round(self) -> None:
        state = {"legal_basis": [], "evidence_agreement": False, "retry_count": 2}
        result = evidence_verification_node(state)
        assert result["retry_count"] == 3
        assert result["should_retry"] is True

    def test_stops_after_max_retries_even_if_insufficient(self) -> None:
    ...
    def test_full_retry_cycle_reaches_max_then_stops(self) -> None:
        retry_count = 0
