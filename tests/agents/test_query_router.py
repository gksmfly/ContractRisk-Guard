...
class _FakeTokenizer:
    ...
    def __init__(self, decode_return: str) -> None:
        self._decode_return = decode_return

    def apply_chat_template(self, messages: Any, tokenize: Any=False, add_generation_prompt: Any=True) -> str:
        return "PROMPT"

    def __call__(self, prompt: Any, return_tensors: Any="pt") -> dict:
        return _FakeBatch({"input_ids": torch.zeros((1, 3), dtype=torch.long)})

    def decode(self, ids: Any, skip_special_tokens: Any=True) -> str:
...
class TestRouteLawNames:
    ...
    def _routing_on(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_no_law_names_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(query_router, "get_law_names", lambda: [])
        assert query_router.route_law_names("아무 조항") is None

    def test_model_load_failure_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_inference_failure_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_happy_path_filters_to_known_law_names(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_all_predictions_unknown_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(query_router, "get_law_names", lambda: ["민법"])
        monkeypatch.setattr(
            query_router,
            "_get_local_model",
    ...
    def test_no_json_in_output_returns_none(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(query_router, "get_law_names", lambda: ["민법"])
        monkeypatch.setattr(
...
class TestEnabledSwitch:
    ...
    def _tripwire(self, monkeypatch: pytest.MonkeyPatch) -> None:
        called = []
        monkeypatch.setattr(query_router, "get_law_names", lambda: ["민법"])
        monkeypatch.setattr(
    ...
    def test_disabled_values_skip_model_load(self, monkeypatch: pytest.MonkeyPatch, value: Any) -> None:
    ...
    def test_enabled_values_run_routing(self, monkeypatch: pytest.MonkeyPatch, value: Any) -> None:
        monkeypatch.setenv("EXAONE_ENABLED", value)
