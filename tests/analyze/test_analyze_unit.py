# tests/analyze/test_analyze_unit.py
"""
...
class TestSplitClauses:
    ...
    def test_too_short_text_returns_empty_list(self) -> None:
        assert split_clauses("안녕") == ([], 0)

    def test_splits_numbered_articles(self) -> None:
        text = (
    ...
    def test_caps_at_max_clauses_and_reports_truncation(self) -> None:
        over = _MAX_CLAUSES + 7
    ...
    def test_realistic_30_clause_contract_is_not_truncated(self) -> None:
    ...
    def test_non_korean_text_treated_as_single_clause(self) -> None:
    ...
    def test_plain_non_clause_text_does_not_crash(self) -> None:
...
class TestExtractSpans:
    def test_empty_evidence_span_returns_empty_list(self) -> None:
        assert _extract_spans("원문 텍스트입니다", "") == []

    def test_evidence_not_in_clause_returns_empty_list(self) -> None:
        assert _extract_spans("원문 텍스트입니다", "존재하지않는문구") == []

    def test_finds_matching_span_with_correct_offsets(self) -> None:
