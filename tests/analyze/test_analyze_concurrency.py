# tests/analyze/test_analyze_concurrency.py
...
class TestConcurrencyLimit:
    async def test_times_out_with_503_when_slot_unavailable(self) -> None:
        async with analyze._analyze_semaphore:
            with pytest.raises(HTTPException) as exc_info:
                await analyze.run_analyze(_TEXT)
        assert exc_info.value.status_code == 503

    async def test_releases_semaphore_after_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
