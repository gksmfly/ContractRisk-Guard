...
//   조항 단위 재현 78.0%     ← "이 조항을 확인하라"
//   조 단위는 상수와 미판정    ← 그래서 조 이름은 단정하지 않는다
//   오경보율은 아직 없다 — 예전 "2.6%"는 준거가 GPT 라벨이라 순환이어서 철회(2026-09-01)
const REVIEW_CFG = {
  label: "확인 필요",
  color: "text-ochre",
  bg: "bg-ochre-soft",
 ...
 * 성능은 상수(항상 {제6,8,9조})와 구분되지 않는다(clean 지층 38.6% vs 36.1%,
 * CI [-3.7,+9.2] 미판정). 조를 정확히 맞혔다는 주장은 근거가 없으므로, 사용자가
 * 조문을 찾아볼 **출발점**으로만 제시한다. 검증된 주장은 조항 지목뿐이다
 * (조항 단위 재현 78.0%; 오경보율은 아직 측정되지 않았다).
 */
